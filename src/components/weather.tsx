...
const home = location.hostname + '/../portfolio/assets/img/home_7.jpg'
const rain = location.hostname + "/../portfolio/assets/img/rain.png"
...
export default function Weather() {
    ...
    fetch("https://api.open-meteo.com/v1/forecast?latitude=45.07&longitude=7.69&hourly=temperature_2m,rain,weathercode,cloudcover&daily=sunrise,sunset&current_weather=true&timezone=Europe%2FBerlin", { cache: 'no-store' })
        .then(res => res.json())
        .then(data => {
            console.log(data.hourly.rain[hour])
            switch (true) {
    ...
    return (<div id="weather" className="min-h-svh w-svw" style={{ backgroundImage: `url(${image})`, backgroundPosition: 'center', backgroundSize: 'cover', backgroundRepeat: 'no-repeat' }}><h1>Asd</h1></div>)
}